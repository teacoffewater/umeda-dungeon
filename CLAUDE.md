...
高低差モデル: 各層の既定の深さを `DEPTH_DEFAULT = { S1: 3, B1: 6, B2: 9 }`(m)と置き、施設の実測 `ZONES[zone].depthM` と階段・坂の途中の点の実測 `NODE_DEPTH`(main.js)があればそれで床・ノードを上下させる(縦の縮尺は 1m=4.4、層間隔の1/5)。両端の深さが違うエッジは床帯にせず斜めの箱で描く(`tools/gen_polys.py` が NODE_DEPTH を読む)。距離計算は水平距離。
