...
- **詳細地図を持つ施設は `detail_maps.js` の `DETAIL_MAPS` に登録したものだけ**（現在 `whity` / `avanza` / `dotica` / `dojima_flat` / `kanden_b1`(仮配置) / `kanden_b2` / `links_b1` / 三番街の館×階4枚）。`enterDetail()` は未登録ゾーンでは何もしない。施設ごとに専用エリア（`origin`）を離して置き、互いに重ねない
