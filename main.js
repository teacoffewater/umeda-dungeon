...
const DEPTH_VIEW = false; // 高低差の表示。true で層の中の高低差を床・ノードに反映(2026-09-03 に「一旦無し」で false。データは残す)
const Y_PER_M = DEPTH_VIEW ? (FLOOR_Y.S1 - FLOOR_Y.B1) / (DEPTH_DEFAULT.B1 - DEPTH_DEFAULT.S1) / 5 : 0; // 表示ON時 4.4(層間隔の1/5)
...
const ZONES = {
  ...
  avanza:      { name: '堂島アバンザ',          color: 0x9a8f52, label: [782.9, 1514.8], depthM: 2.4 }, // オリーブ。深さはドーチカ6m−階段(7+17段×15cm=3.6m)(現地 2026-09-03)
  ...
  kiyo:        { name: '紀陽ビル',              color: 0x7a6bc9, label: [681, 1614] }, // B1F(ドーチカ南端直結。福永診療所・堂島デンタルクリニック。現地 2026-09-03)
...
const NODES = [
  ...
  // ドーチカ(ドージマ地下センター)の本線・枝・接続ビルの位置は、南端(C93)の「地下近辺案内」板(2026-09-03 現地撮影)を
  // OSM のビル外形に当てて metric-v1 に変換したもの(tools/extract_dotica_board.py → tools/data/dotica_board.json)。
  // 本線のノードは板の白い通路の軸上、枝(C61/C72/C80/C83/C84)は板の通路の位置。以前の OSM 中心線(@region dotica)は廃止
  P('dojima',      'ドージマ地下センター',       'B1',  703.6, 1444.1, 'dotica'),
  P('avanza',      '堂島アバンザ前',             'B1',  703.5, 1552.7, 'dotica'),
  P('dojima_flat', '堂島ふらっと (近鉄堂島ビルB1F)', 'B1', 649.7, 1532.6, 'dojima_flat'), // ドーチカ C83 の通路の西端(板)。階段を上がった所(現地 2026-09-03)
  // ドーチカ⇔堂島ふらっとの階段(現地の2点タップ 2026-09-03、tools/data/survey/2026-09-03_dotica_dojima_flat.json。位置は板の C83 通路の西端に置き直し)。
  // 23段(≈3.9m上り)。ドーチカから見て左が階段、右(北側)に上下2本のエスカレーター。同じ「中枢層」扱いなので昇降設備にはせず、通路の折れ点として持つ
  J('dotica_df_s0', 663.8, 1532.8), // 階段の下(ドーチカ側)
  J('dotica_df_s1', 656.4, 1532.7),   // 階段の上(堂島ふらっと側)
  P('kanden_b1',   '関電不動産西梅田ビル B1F',    'S1', 679.3, 1388.2, 'kanden'), // 館内階段でB2Fと(現地 2026-09-03)
  P('kanden_b2',   '関電不動産西梅田ビル B2F',    'B1', 679.3, 1388.2, 'kanden'), // ドーチカ C61 の通路(板)を西へ入った所(北東角)
  P('kiyo_b1',     '紀陽ビル B1F',              'B1', 688, 1613, 'kiyo'),   // ドーチカ南端(現在地・トイレの所)から西へ入った所(現地 2026-09-03)
  ...
  J('j_avz_n',      763.2, 1468.2),  // 北: 階段の上(アバンザ北サンクンガーデン通路)
  ...
  // @region whity end
  // ドーチカ本線の折れ点・分岐点(地下近辺案内板 2026-09-03。tools/data/dotica_board.json の nodes)
  J('dotica_02',  713, 1359.6),  // 北端(曽根崎地下歩道 j_sone_w との境)
  J('dotica_c61', 709, 1388.4),  // C61 の通路(西→関電不動産西梅田ビル B2F)の分岐点
  J('dotica_01',  703.9, 1544.2),  // C84 の通路(東→アバンザ南)の分岐点
  J('dotica_c92', 713.1, 1599.5),  // C92 の所の折れ(本線が東へ寄る)
  J('dotica_03',  704.8, 1613.5),  // 南端(C93 の手前。板の通路はここまで)
...
const EDGES = [
  ...
  ['dotica_c61', 'kanden_b2', 5, 'dotica'],     // C61: ドーチカ北側の西へ出る通路→関電不動産西梅田ビル B2F(北東角)
  ['dotica_03', 'kiyo_b1', 5, 'dotica'],        // 南端: ドーチカ(現在地・トイレの所)→紀陽ビル B1F(現地 2026-09-03)
  ...
  // @region whity end
  // ドーチカ本線(北→南)。地下近辺案内板(2026-09-03)の通路の軸。床は tools/data/dotica_board.json の外形(gen_polys.py)
  ['dotica_02', 'dotica_c61', 12, 'dotica'], ['dotica_c61', 'dojima', 12, 'dotica'],
  ['dojima', 'dotica_avz_n', 12, 'dotica'], ['dotica_avz_n', 'dotica_avz_c', 12, 'dotica'],
  ['dotica_avz_c', 'dotica_c83', 12, 'dotica'], ['dotica_c83', 'dotica_01', 12, 'dotica'],
  ['dotica_01', 'avanza', 12, 'dotica'], ['avanza', 'dotica_c92', 12, 'dotica'], ['dotica_c92', 'dotica_03', 12, 'dotica'],
...
const VERTICALS = [
  ...
  { type: 'stairs', a: 'kanden_b1',   b: 'kanden_b2',   mx: 677, my: 1391, name: '関電不動産西梅田ビル館内階段' }, // B2案内板の「現在地」横の階段(東側)
...
const FLOOR_POLYS = [
  ...
  { floor: 'S1', zone: 'umekita', pts: [[596.7, 773.6], [594.6, 810.2], [600.2, 813.6], [609.1, 814.2], [616.7, 809.5], [628.5, 794.8], [635.6, 775.8], [636.4, 766.9], [631.6, 754.6], [629.2, 754.0], [631.6, 747.4], [624.8, 742.6], [620.7, 754.2], [610.6, 757.1]], covers: [['umekita_sq', 'grandfront']] },
  ...
  { floor: 'S1', zone: 'kanden', pts: [[696.7, 1394.7], [698.5, 1371.1], [687.3, 1357.8], [665.2, 1355.6], [639.5, 1366.8], [641.4, 1380.1], [643.5, 1379.6], [647.6, 1402.3]] },
  ...
  { floor: 'S1', zone: 'osaka_sta', pts: [[512.9, 984.1], [562.3, 1066.3], [618.8, 1032.6], [620.5, 1035.2], [625.6, 1032.3], [567.3, 935.1], [560.9, 939.1], [547.2, 917.2], [536.6, 923.5], [440.3, 998.5], [454.2, 1019.6], [492.9, 994.4], [493.9, 995.9]] },
  { floor: 'S1', zone: 'osaka_sta', pts: [[679.7, 765.2], [662.8, 762.2], [670.8, 771.2], [678.9, 792.0], [654.4, 810.5], [655.3, 812.7], [665.4, 806.5], [666.4, 807.8], [699.8, 787.1]] },
  { floor: 'S1', zone: 'osaka_sta', pts: [[838.3, 723.5], [829.8, 727.2], [828.3, 725.0], [807.9, 736.7], [817.1, 732.0], [820.0, 746.5], [835.3, 737.2], [837.4, 738.7], [833.5, 732.7], [846.7, 727.3], [843.0, 707.1], [846.7, 706.9], [846.3, 703.9], [864.9, 701.8], [842.6, 702.8], [849.1, 698.3], [853.4, 692.7], [845.6, 692.8], [843.4, 699.2], [835.3, 708.2]] },
  { floor: 'S1', zone: 'osaka_sta', pts: [[755.4, 786.1], [757.1, 780.4], [751.8, 784.6], [749.0, 784.6], [733.8, 797.9], [734.4, 798.9]] },
  { floor: 'S1', zone: 'osaka_sta', pts: [[778.7, 769.3], [770.3, 775.9], [769.4, 771.3], [777.7, 763.9], [762.3, 776.2], [759.4, 783.6], [768.2, 778.3]] },
  { floor: 'S1', zone: 'ekimae', pts: [[873.7, 1330.6], [881.9, 1339.6], [942.3, 1344.6], [989.8, 1344.1], [998.1, 1333.2], [988.2, 1302.4], [1002.2, 1302.3], [1013.1, 1336.8], [1024.1, 1344.7], [1105.2, 1344.8], [1115.6, 1333.8], [1090.4, 1259.0], [1081.2, 1250.5], [1042.1, 1250.9], [1032.1, 1217.5], [1070.8, 1217.5], [1078.9, 1208.2], [1055.8, 1139.2], [1042.6, 1135.0], [956.1, 1159.3], [951.1, 1171.6], [972.2, 1212.2], [981.9, 1217.6], [1023.8, 1217.5], [1033.8, 1250.9], [1027.4, 1251.0], [1027.1, 1246.7], [1024.2, 1246.5], [1024.4, 1235.4], [1022.6, 1235.6], [1023.6, 1245.6], [1020.0, 1245.6], [1019.9, 1243.1], [1019.4, 1245.6], [1015.2, 1245.6], [1015.5, 1246.8], [1019.3, 1246.8], [1019.0, 1251.1], [998.1, 1251.3], [989.7, 1263.6], [999.6, 1294.3], [985.5, 1294.5], [973.9, 1259.7], [964.3, 1250.8], [883.3, 1251.3], [874.4, 1261.0], [874.1, 1292.0], [860.9, 1291.3], [861.8, 1261.0], [853.2, 1251.1], [740.5, 1252.0], [727.5, 1264.5], [725.4, 1310.6], [741.4, 1327.2], [851.0, 1335.6], [859.8, 1327.5], [860.7, 1299.3], [874.0, 1300.0]], covers: [['j_fashion_w', 'ekimae1'], ['j_fashion_e', 'ekimae4'], ['ekimae1', 'ekimae2'], ['ekimae2', 'ekimae3'], ['ekimae3', 'ekimae4']] },
  { floor: 'S1', zone: '_neutral', pts: [[847.3, 678.0], [845.6, 692.6], [853.6, 692.7], [849.2, 698.4], [843.1, 702.6], [877.7, 701.3], [883.3, 705.0], [886.0, 705.0], [884.2, 700.1], [890.4, 700.5], [895.2, 716.1], [897.1, 714.0], [892.9, 700.4], [920.4, 698.2], [910.3, 698.4], [911.9, 692.5], [923.3, 692.3], [922.9, 690.0], [890.7, 693.3], [888.9, 687.5], [922.4, 687.0], [922.2, 685.4], [934.2, 683.6], [933.0, 676.9]], holes: [[[888.2, 693.4], [876.8, 693.9], [875.7, 687.7], [886.4, 687.5]]], covers: [['sanban_s_b1', 'j_yodo_e'], ['yodobashi', 'j_kita']] },
  { floor: 'S1', zone: '_neutral', pts: [[802.4, 750.3], [805.8, 755.2], [819.8, 746.6], [817.0, 732.2], [808.3, 737.1], [808.4, 734.3], [800.4, 734.0], [800.7, 740.5], [797.3, 742.4], [799.3, 745.5], [790.6, 753.6], [787.4, 737.2], [779.2, 742.2], [782.6, 760.0], [769.5, 771.4], [770.4, 775.6], [783.8, 766.1], [784.3, 768.4], [792.5, 763.4], [791.8, 759.8]] },
  { floor: 'B1', zone: 'sanban', pts: [[905.6, 468.1], [894.8, 492.0], [909.6, 572.3], [936.0, 567.5], [936.0, 577.7], [911.6, 582.1], [913.2, 590.4], [911.4, 590.8], [915.9, 617.6], [918.3, 617.2], [925.9, 662.5], [929.0, 662.0], [929.8, 667.3], [931.3, 667.1], [934.4, 683.7], [922.3, 685.5], [929.7, 727.7], [984.2, 719.7], [1002.4, 715.2], [1030.8, 704.5], [1046.3, 695.9], [1042.6, 672.4], [1033.0, 673.9], [1030.8, 660.1], [1039.9, 658.6], [1031.5, 608.4], [1020.1, 563.4], [990.0, 567.8], [990.0, 557.7], [1017.5, 553.7], [1016.6, 550.6], [1018.5, 550.1], [1017.5, 545.7], [1018.6, 545.5], [989.4, 444.3], [913.3, 457.9], [911.3, 467.3]], holes: [[[986.0, 568.4], [982.9, 568.9], [940.0, 577.0], [940.0, 566.8], [975.7, 559.9], [986.0, 558.3]]], covers: [['hankyu', 'sanban_n_b1'], ['sanban_n2_w', 'sanban_s2_w'], ['sanban_n2_e', 'sanban_s2_e']] },
  ...
  { floor: 'B1', zone: 'ema', pts: [[958.1, 1078.8], [956.0, 1079.9], [977.2, 1135.1], [1013.3, 1121.6], [1004.9, 1100.9], [986.1, 1068.1], [982.4, 1069.7], [979.6, 1060.6], [956.6, 1074.8]], covers: [['j_market_ne', 'ema']] },
  { floor: 'B1', zone: 'hep', pts: [[1090.5, 760.2], [1077.9, 863.8], [1080.5, 869.8], [1085.0, 871.8], [1091.2, 869.9], [1160.1, 804.1], [1155.3, 762.2], [1151.1, 763.0], [1151.1, 761.0], [1128.9, 763.3], [1128.3, 756.5], [1145.6, 754.8], [1143.9, 730.1], [1205.1, 725.5], [1201.3, 667.9], [1191.9, 669.0], [1176.3, 675.5], [1112.5, 706.4], [1090.1, 720.3], [1089.8, 729.6], [1086.6, 735.9], [1086.1, 742.5], [1088.2, 742.5], [1086.3, 760.6]], covers: [['j_east1', 'hep'], ['j_nm2', 'navio']] },
  { floor: 'B1', zone: 'osbld', pts: [[1112.7, 873.3], [1115.1, 879.2], [1162.6, 879.5], [1162.7, 876.2], [1168.9, 873.1], [1171.5, 867.8], [1174.8, 867.9], [1176.4, 817.7], [1167.7, 817.4]], covers: [['j_nm2', 'osbld']] },
  ...
  { floor: 'B1', zone: 'diamor', pts: [[695.5, 1225.5], [702.7, 1226.0], [710.0, 1230.6], [710.0, 1234.6], [716.7, 1234.6], [716.4, 1245.9], [719.7, 1245.9], [717.2, 1261.6], [719.7, 1267.4], [720.9, 1245.9], [724.5, 1245.9], [725.0, 1234.6], [853.1, 1234.2], [833.4, 1251.8], [847.0, 1251.7], [861.0, 1239.1], [859.3, 1327.2], [858.7, 1331.7], [857.0, 1331.6], [856.9, 1341.0], [855.1, 1341.4], [857.9, 1341.7], [856.3, 1358.4], [861.7, 1359.3], [861.4, 1373.0], [867.6, 1372.8], [867.9, 1359.5], [876.6, 1360.0], [877.5, 1346.1], [874.1, 1345.8], [874.2, 1342.4
1218.6], [710.0, 1218.6], [710.0, 1224.5], [703.7, 1220.5], [703.0, 1215.4], [699.2, 1215.5], [699.5, 1220.2], [695.3, 1220.0]], holes: [[[997.9, 1237.2], [997.4, 1232.8], [1016.3, 1232.9], [1016.2, 1236.3]], [[994.7, 1237.3], [981.4, 1238.0], [979.7, 1232.7], [994.1, 1232.8]], [[918.4, 1239.6], [921.2, 1232.9], [965.1, 1232.7], [966.9, 1238.6]], [[880.4, 1152.0], [882.7, 1152.0], [883.6, 1153.7], [896.6, 1147.1], [927.9, 1168.6], [937.9, 1185.6], [932.8, 1189.3], [937.3, 1196.9], [943.7, 1193.5], [951.4, 1206.5], [945.6, 1206.6], [945.7, 1214.1], [957.8, 1213.9], [949.4, 1217.8], [949.9, 1218.8], [876.9, 1219.1], [877.5, 1149.3]], [[880.7, 1233.1], [916.6, 1232.9], [915.0, 1239.7], [886.9, 1240.3]], [[910.9, 1148.3], [888.4, 1132.4], [887.2, 1130.1], [888.7, 1128.0], [912.6, 1145.9]], [[895.6, 1108.0], [898.5, 1113.3], [889.9, 1111.4]], [[855.5, 1130.3], [850.9, 1128.2], [844.8, 1131.5], [841.3, 1125.8], [857.1, 1123.5], [862.0, 1125.7]], [[824.9, 1069.5], [828.1, 1067.4], [831.2, 1074.8], [834.5, 1072.7], [847.9, 1098.9], [839.8, 1105.6], [831.7, 1106.0], [826.6, 1099.9], [815.9, 1079.4], [824.9, 1073.5]]], covers: [['hanshin', 'j_diamor_n'], ['j_diamor_s', 'ekimae1_b2'], ['j_diamor_s', 'ekimae2_b2'], ['enkei', 'j_hilton_e'], ['j_fashion_w', 'j_diamor_s'], ['diamor_01', 'j_sone_c'], ['diamor_03', 'diamor_00'], ['diamor_03', 'diamor_05'], ['diamor_03', 'j_diamor_s'], ['diamor_03', 'j_market_ne'], ['diamor_04', 'diamor_03'], ['j_diamor_e', 'j_fashion_e'], ['j_diamor_e', 'j_variety'], ['j_diamor_s', 'j_diamor_e'], ['j_diamor_s', 'j_sone_c'], ['j_sone_c', 'diamor_02'], ['j_variety', 'diamor_04'], ['sonechika_06', 'j_diamor_e'], ['diamor_03', 'enkei'], ['enkei', 'j_diamor_n']] },
  { floor: 'B1', zone: 'whity', pts: [[900.3, 950.1], [913.3, 949.6], [938.8, 997.5], [955.1, 996.9], [969.7, 1001.0], [971.6, 998.4], [976.2, 1003.7], [982.7, 1006.9], [1019.1, 1037.1], [1047.5, 1062.8], [1052.9, 1071.1], [1054.5, 1069.8], [1056.2, 1074.6], [1057.2, 1074.3], [1053.3, 1066.2], [1055.5, 1064.0], [1058.9, 1064.1], [1062.1, 1067.3], [1073.5, 1063.3], [1079.5, 1085.9], [1084.4, 1085.4], [1089.9, 1109.8], [1101.9, 1106.3], [1111.0, 1100.8], [1110.1, 1098.0], [1107.6, 1098.6], [1095.9, 1067.4], [1100.2, 1065.9], [1100.2, 1069.1], [1108.1, 1068.7], [1108.0, 1062.0], [1100.2, 1062.3], [1100.2, 1064.0], [1095.0, 1065.2], [1093.
.5, 747.7], [1047.9, 749.2], [1053.4, 765.5], [1061.6, 761.4], [1068.4, 801.6], [1065.8, 833.2], [1051.8, 833.6], [1051.8, 852.2], [1044.1, 852.9], [1044.4, 855.3], [1034.9, 858.5], [1037.0, 866.5], [1050.7, 864.2], [1049.8, 861.4], [1063.6, 859.2], [1061.2, 888.8], [1050.2, 887.7], [1046.4, 918.2], [1058.2, 919.9], [1057.5, 923.5], [1054.2, 933.7], [1025.6, 936.9], [1024.9, 923.1], [988.6, 924.6], [988.1, 923.1], [971.3, 925.3], [965.0, 918.2], [950.5, 926.0], [922.0, 927.2], [915.8, 926.1], [915.8, 927.1], [911.6, 926.9], [906.6, 917.8], [898.8, 922.3], [901.8, 928.1], [899.4, 928.2]], holes: [[[1284.4, 923.8], [1280.1, 928.4], [1273.2, 929.0], [1276.5, 922.6], [1275.1, 921.5], [1283.8, 918.4]], [[1153.7, 931.7], [1111.1, 935.5], [1110.4, 924.7], [1153.3, 924.1]], [[1103.4, 936.2], [1094.1, 936.8], [1091.1, 932.1], [1103.2, 932.4]], [[1092.3, 918.8], [1095.7, 919.2], [1091.1, 932.1], [1088.7, 928.3]], [[1058.6, 651.0], [1056.2, 634.6], [1057.6, 634.2], [1063.4, 649.2]], [[1055.0, 991.6], [1050.7, 993.1], [1048.4, 984.6], [1052.9, 983.1]], [[1046.8, 962.1], [1050.7, 963.0], [1045.5, 972.0]], [[1046.3, 1030.5], [1042.4, 1031.7], [1041.8, 1028.3], [1045.0, 1027.6]], [[1008.6, 986.8], [1010.2, 997.7], [1023.0, 995.9], [1026.4, 1016.5], [996.0, 989.0]], [[991.2, 969.5], [1015.2, 965.0], [1017.9, 970.6], [994.1, 974.3], [996.1, 990.9], [995.1, 991.1], [991.2, 981.8], [988.4, 982.1], [977.6, 972.3], [989.4, 969.9], [991.2, 972.8]], [[982.2, 953.8], [978.9, 946.9], [987.8, 946.5]], [[933.4, 958.2], [936.4, 956.6], [943.6, 969.8], [950.7, 968.9], [956.3, 971.4], [957.3, 976.3], [958.5, 976.1], [962.7, 985.9], [957.6, 981.0], [945.9, 991.7], [926.1, 954.4], [929.9, 952.0]]], covers: [['sanban_s_b2', 'opa'], ['sanban_s_b2', 'j_east1'], ['sanban_s_b1', 'opa'], ['j_pchn', 'opa'], ['j_metro', 'whity_w'], ['j_east1', 'whity_20'], ['j_east1', 'whity_26'], ['j_f40', 'whity_19'], ['j_f40', 'whity_25'], ['j_hanshin_e', 'whity_04'], ['j_higashi_n', 'whity_30'], ['j_nm1s', 'whity_32'], ['whity_02', 'whity_01'], ['whity_02', 'whity_03'], ['whity_03', 'whity_08'], ['whity_03', 'whity_14'], ['whity_04', 'whity_08'], ['whity_05', 'whity_04'], ['whity_07', 'whity_02'], ['whity_07', 'whity_06'], ['whity_08', 'whity_11'], ['whity_09', 'whity_07'], ['whity_11', 'whity_10'], ['whity_11', 'whity_12'], ['whity_12', 'whity_15'], ['whity_12', 'whity_21'], ['whity_14', 'whity_13'], ['whity_14', 'whity_30'], ['whity_15', 'j_f40'], ['whity_17', 'whity_16'], ['whity_20', 'whity_17'], ['whity_20', 'whity_18'], ['whity_21', 'whity_14'], ['whity_21', 'whity_33'], ['whity_23', 'whity_22'], ['whity_24', 'whity_23'], ['whity_27', 'whity_28'], ['whity_28', 'whity_34'], ['whity_29', 'whity_21'], ['whity_30', 'whity_32'], ['whity_30', 'whity_40'], ['whity_31', 'whity_23'], ['whity_31', 'whity_34'], ['whity_32', 'j_whity_x'], ['whity_32', 'whity_31'], ['whity_33', 'j_higashi_n'], ['whity_34', 'whity_35'], ['whity_35', 'whity_37'], ['whity_36', 'whity_33'], ['whity_36', 'whity_39'], ['whity_37', 'j_east1'], ['whity_38', 'whity_36'], ['whity_40', 'izumi_ne'], ['opa', 'whity_17'], ['whity_w', 'whity_13'], ['izumi', 'whity_40'], ['j_nm2', 'j_nm1s']] },
  { floor: 'B1', zone: 'whity', pts: [[1040.8, 761.4], [1038.7, 762.8], [1044.7, 769.9], [1041.1, 772.1], [1039.1, 769.5], [1034.1, 772.6], [1028.8, 764.0], [1025.1, 766.4], [1032.2, 778.9], [1051.8, 766.5], [1047.1, 758.6], [1041.5, 762.3]] },
  { floor: 'B1', zone: 'umechika', pts: [[896.5, 792.6], [902.8, 809.0], [911.1, 807.1], [909.4, 802.9], [938.8, 807.7], [921.8, 818.4], [910.3, 822.9], [911.7, 826.4], [894.0, 828.8], [890.5, 833.2], [893.3, 841.9], [888.1, 842.3], [891.9, 852.1], [891.9, 863.1], [870.5, 863.3], [875.4, 868.9], [893.5, 868.4], [899.7, 866.8], [901.0, 860.9], [911.7, 860.6], [911.5, 852.8], [907.1, 852.5], [901.5, 839.1], [903.5, 835.9], [921.6, 833.4], [942.0, 889.1], [942.3, 896.3], [944.9, 897.2], [955.3, 895.7], [951.1, 806.3], [1009.8, 768.6], [1016.1, 770.6], [1020.0, 776.4], [1019.8, 781.1], [1021.6, 781.4], [1019.8, 782.0], [1019.7, 804.8], [1050.1, 801.3], [1049.1, 768.3], [1032.2, 779.0], [1024.9, 766.4], [1028.8, 763.8], [1034.1, 772.4], [1039.1, 769.3], [1041.1, 771.9], [1044.5, 769.9], [1038.6, 762.7], [1047.0, 758.5], [1010.6, 722.4], [951.0, 724.5], [929.7, 727.8], [929.1, 725.3], [905.7, 726.1], [908.0, 797.2], [904.6, 789.8], [898.3, 790.2], [896.4, 790.7]], holes: [[[940.4, 853.8], [928.5, 820.9], [938.6, 814.4]]], covers: [['sanban_s_b1', 'j_shibata'], ['hanshin', 'j_hanshin_e'], ['j_market_ne', 'j_hanshin_e'], ['j_market_ne', 'umechika_35'], ['j_metro', 'umechika_15'], ['j_mido_n', 'umechika_16'], ['j_shibata', 'umechika_21'], ['j_sun', 'umechika_19'], ['umechika_04', 'umechika_02'], ['umechika_04', 'umechika_03'], ['umechika_06', 'umechika_04'], ['umechika_07', 'umechika_04'], ['umechika_08', 'umechika_04'], ['umechika_10', 'umechika_11'], ['umechika_13', 'umechika_19'], ['umechika_15', 'umechika_11'], ['umechika_16', 'umechika_12'], ['umechika_18', 'umechika_20'], ['umechika_19', 'umechika_14'], ['umechika_19', 'umechika_20'], ['umechika_20', 'umechika_21'], ['umechika_20', 'umechika_22'], ['umechika_21', 'umechika_26'], ['umechika_24', 'umechika_17'], ['umechika_26', 'umechika_25'], ['umechika_27', 'umechika_26'], ['umechika_28', 'umechika_16'], ['umechika_29', 'whity_05'], ['umechika_31', 'umechika_30'], ['umechika_32', 'umechika_31'], ['umechika_33', 'umechika_23'], ['umechika_34', 'umechika_29'], ['umechika_23', 'sanban_s_b2'], ['sanban_s_b2', 'umechika_24'], ['umechika_26', 'hq_concourse'], ['hq_concourse', 'umechika_31'], ['umechika_05', 'umechika_p4_daimaru'], ['umechika_p4_daimaru', 'umechika_01'], ['daimaru', 'umechika_p4_daimaru'], ['umechika_11', 'umechika_p4_hanshin'], ['umechika_p4_hanshin', 'umechika_09'], ['hanshin', 'umechika_p4_hanshin'], ['j_sun', 'j_mido_n']] },
  { floor: 'B1', zone: 'umechika', pts: [[834.2, 1012.4], [835.4, 1014.4], [849.6, 1006.3], [850.8, 1000.0], [866.7, 1003.0], [864.9, 1013.2], [924.3, 1002.1], [938.7, 997.5], [913.2, 949.8], [900.1, 950.2], [899.2, 928.2], [901.6, 927.9], [898.6, 922.3], [901.5, 920.6], [838.9, 922.8], [840.8, 934.1], [856.4, 960.7], [856.0, 972.4], [847.9, 982.7], [850.1, 986.6], [826.3, 1002.8], [823.7, 997.8], [803.2, 1010.4], [807.8, 1023.8], [816.8, 1033.9], [820.2, 1015.7], [822.4, 1018.8], [824.4, 1018.5]], holes: [[[906.7, 950.2], [908.8, 953.4], [902.2, 957.2], [899.0, 955.3]]] },
  { floor: 'B1', zone: 'umechika', pts: [[759.3, 955.2], [759.5, 941.1], [766.9, 952.8], [783.7, 942.5], [779.5, 934.8], [784.3, 931.6], [784.3, 935.6], [792.1, 935.5], [792.2, 926.5], [821.3, 907.3], [816.0, 899.3], [861.0, 869.3], [864.3, 869.2], [859.3, 863.5], [847.4, 870.7], [845.9, 868.3], [842.8, 870.3], [844.2, 872.7], [799.5, 902.5], [797.8, 899.5], [794.6, 901.6], [796.4, 904.6], [767.6, 924.8], [764.5, 921.1], [762.2, 922.2], [755.7, 928.5], [758.2, 931.2], [751.6, 934.2], [751.9, 935.6], [730.9, 950.3], [729.6, 948.0], [726.5, 950.1], [727.8, 952.4], [726.5, 953.3], [729.2, 958.1], [753.2, 941.8], [754.3, 947.1], [751.7, 947.2], [751.6, 955.3]] },
  { floor: 'B1', zone: 'umechika', pts: [[753.5, 915.8], [753.5, 913.1], [773.0, 900.9], [766.6, 890.7], [745.8, 903.8], [741.5, 903.8], [741.5, 906.0], [721.7, 910.5], [723.9, 920.3], [744.1, 915.7], [745.6, 918.1], [749.2, 915.8]] },
  { floor: 'B1', zone: 'umechika', pts: [[943.6, 970.0], [936.3, 956.8], [933.4, 958.4], [929.9, 952.2], [926.3, 954.5], [946.0, 991.5], [957.6, 980.9], [962.5, 985.8], [958.4, 976.3], [957.2, 976.4], [956.2, 971.5], [950.6, 969.1]] },
  { floor: 'B1', zone: 'umechika', pts: [[911.2, 926.2], [950.5, 925.9], [964.7, 918.3], [908.1, 920.3]] },
  { floor: 'B1', zone: 'umechika', pts: [[991.4, 972.8], [989.4, 970.1], [977.9, 972.4], [988.5, 982.0], [991.4, 981.7], [995.2, 990.9], [994.0, 974.2], [1017.7, 970.5], [1015.1, 965.2], [991.3, 969.7]] },
  { floor: 'B1', zone: 'umechika', pts: [[896.7, 812.5], [896.2, 800.3], [883.3, 799.0], [883.8, 811.3]] },
  { floor: 'B1', zone: 'umechika', pts: [[788.6, 1041.4], [790.3, 1040.4], [793.6, 1021.8], [781.8, 1029.2]] },
  { floor: 'B1', zone: 'umechika', pts: [[1008.5, 987.0], [996.4, 989.1], [1005.6, 997.6], [1010.0, 997.4]] },
  { floor: 'B1', zone: 'osaka_sta', pts: [[684.4, 974.4], [732.7, 918.4], [723.9, 920.4], [721.7, 911.3], [635.2, 1011.5], [639.5, 1015.2], [622.1, 1036.5], [627.1, 1044.6]], covers: [['jr_osaka', 'daimaru'], ['daimaru', 'j_c1'], ['daimaru', 'kitte']] },
  { floor: 'B1', zone: 'osaka_sta', pts: [[674.2, 988.5], [689.7, 1017.6], [692.7, 1015.7], [695.2, 1020.4], [705.7, 1019.2], [688.0, 986.1], [729.1, 958.2], [726.4, 953.3]] },
  ...
  { floor: 'B1', zone: 'ekimae', pts: [[989.7, 1263.6], [1013.1, 1336.8], [1024.3, 1344.1], [1094.6, 1344.9], [1105.2, 1350.9], [1104.5, 1352.7], [1129.6, 1353.2], [1137.5, 1350.1], [1103.7, 1346.5], [1112.9, 1336.7], [1114.2, 1338.6], [1116.0, 1338.2], [1132.5, 1347.5], [1134.0, 1345.0], [1137.9, 1346.3], [1138.8, 1349.0], [1141.0, 1344.4], [1126.3, 1334.8], [1123.0, 1325.8], [1109.8, 1318.3], [1119.8, 1316.9], [1110.1, 1317.7], [1090.4, 1259.0], [1081.2, 1250.5], [1042.1, 1250.9], [1032.1, 1217.5], [1070.8, 1217.5], [1078.9, 1208.2], [1055.8, 1139.2], [1042.6, 1135.0], [956.1, 1159.3], [951.1, 1171.6], [972.2, 1212.2], [975.8, 1214.2], [973.4, 1216.5], [975.4, 1218.5], [995.5, 1216.9], [1023.8, 1217.5], [1024.2, 1218.8], [1029.5, 1218.9], [1030.2, 1232.3], [1028.4, 1233.1], [1033.8, 1250.9], [998.1, 1251.3]], holes: [[[1019.2, 1251.4], [1019.4, 1251.7], [1017.4, 1251.6]]], covers: [['j_diamor_e', 'ekimae4_b2'], ['j_fashion_w', 'ekimae1'], ['j_fashion_e', 'ekimae4'], ['ekimae1_b2', 'j_sone_w'], ['ekimae1_b2', 'j_sone_c'], ['ekimae3_b2', 'sonechika'], ['ekimae1_b2', 'ekimae2_b2'], ['ekimae2_b2', 'ekimae3_b2'], ['ekimae3_b2', 'ekimae4_b2'], ['kitashinchi', 'ekimae2_b2'], ['kitashinchi', 'ekimae1_b2']] },
  { floor: 'B1', zone: 'ekimae', pts: [[708.2, 1345.4], [708.2, 1334.4], [717.4, 1334.4], [717.5, 1348.2], [733.1, 1339.3], [719.5, 1339.8], [741.4, 1327.2], [824.5, 1333.5], [824.5, 1335.8], [816.5, 1336.1], [818.6, 1337.9], [803.9, 1337.0], [836.7, 1339.3], [838.3, 1341.5], [848.1, 1341.6], [857.0, 1349.1], [857.7, 1341.8], [855.0, 1341.3], [856.7, 1340.9], [856.9, 1331.6], [859.2, 1327.2], [860.3, 1259.2], [853.2, 1251.1], [740.5, 1252.0], [727.5, 1264.5], [726.2, 1293.7], [715.9, 1296.5], [721.9, 1310.4], [713.4, 1315.9], [717.0, 1324.1], [717.3, 1314.0], [727.3, 1312.5], [735.7, 1321.3], [716.7, 1332.2], [716.9, 1324.8], [706.0, 1331.2], [703.4, 1334.9], [703.1, 1341.5]] },
  { floor: 'B1', zone: 'ekimae', pts: [[874.4, 1330.3], [882.2, 1338.9], [887.6, 1340.0], [942.3, 1344.6], [986.0, 1344.1], [991.2, 1342.3], [997.4, 1333.1], [973.2, 1259.0], [964.1, 1251.5], [883.6, 1252.0], [880.7, 1254.1], [875.0, 1261.2]] },
  { floor: 'B1', zone: 'sonechika', pts: [[1021.6, 1370.5], [1025.6, 1370.6], [1025.3, 1367.4], [1040.6, 1368.0], [1040.7, 1381.5], [1066.7, 1381.5], [1066.7, 1372.5], [1049.7, 1372.5], [1049.6, 1368.4], [1110.8, 1369.1], [1147.7, 1373.4], [1152.4, 1385.5], [1160.9, 1382.6], [1153.9, 1362.6], [1154.0, 1358.9], [1152.6, 1358.9], [1151.3, 1355.2], [1152.5, 1353.8], [1148.9, 1348.6], [1147.1, 1349.3], [1141.2, 1344.4], [1138.9, 1349.1], [1137.8, 1346.5], [1134.0, 1345.2], [1132.5, 1347.7], [1116.3, 1338.6], [1115.7, 1339.9], [1126.5, 1344.9], [1126.5, 1348.5], [1128.1, 1348.7], [1128.4, 1345.5], [1132.9, 1349.3], [1137.7, 1350.1], [1129.6, 1353.4], [1105.0, 1352.9], [1112.1, 1356.1], [1071.5, 1355.3], [1070.8, 1350.4], [1096.3, 1350.2], [1104.1, 1352.6], [1105.0, 1350.9], [1094.5, 1345.0], [1029.1, 1344.9], [1030.9, 1351.0], [1020.4, 1350.5], [1022.8, 1358.6], [1022.6, 1363.6], [1020.7, 1367.3], [1016.3, 1367.1], [1021.3, 1367.4]], covers: [['j_sone_w', 'j_sone_c'], ['j_sone_c', 'j_sone_e'], ['j_sone_e', 'sonechika_08'], ['j_sone_e', 'sonechika_13'], ['sonechika_01', 'sonechika_03'], ['sonechika_02', 'sonechika_04'], ['sonechika_03', 'sonechika_06'], ['sonechika_04', 'sonechika_03'], ['sonechika_04', 'sonechika_07'], ['sonechika_06', 'j_sone_e'], ['sonechika_06', 'sonechika_05'], ['sonechika_08', 'sonechika_09'], ['sonechika_11', 'sonechika_10'], ['sonechika_13', 'sonechika_12'], ['sonechika_14', 'sonechika_13'], ['sonechika_12', 'sonechika_p1_sonechika'], ['sonechika_p1_sonechika', 'sonechika_11'], ['sonechika', 'sonechika_p1_sonechika']] },
[288.4, 1379.0], [280.4, 1378.6], [281.0, 1386.8], [287.0, 1387.1], [270.9, 1393.8], [272.6, 1397.8], [243.4, 1408.5], [238.1, 1400.0], [231.3, 1404.2], [235.7, 1411.3], [220.1, 1417.0], [224.6, 1429.2], [229.4, 1427.4], [223.4, 1439.1], [230.5, 1442.8], [239.2, 1425.9]], holes: [[[736.1, 1103.2], [727.6, 1102.5], [728.1, 1097.4], [735.1, 1098.7], [741.3, 1110.2], [739.4, 1122.4], [726.7, 1124.3], [727.0, 1116.4], [736.0, 1117.2]], [[733.7, 1158.5], [725.3, 1157.7], [725.9, 1142.6], [736.5, 1141.0]], [[725.8, 1049.0], [731.4, 1059.9], [719.6, 1066.5], [713.4, 1056.4]], [[716.6, 1147.6], [716.8, 1144.0], [720.7, 1143.4], [720.5, 1147.7]], [[715.1, 1161.3], [715.0, 1158.5], [720.3, 1158.0], [720.4, 1162.5]], [[650.5, 1125.4], [654.8, 1135.0], [642.4, 1136.9], [639.9, 1132.2]], [[394.0, 1347.6], [389.4, 1351.2], [384.4, 1344.8], [407.7, 1326.4], [411.0, 1331.0], [394.6, 1343.7], [393.2, 1346.1]]], covers: [['j_hilton_e', 'j_nishi_x'], ['j_nishi_x', 'hilton'], ['hilton', 'garden'], ['j_c1', 'j_kitte_e'], ['garden', 'herbis'], ['garden', 'kitte'], ['herbis', 'j_nishi_x'], ['j_nishi_x', 'j_sone_w'], ['j_kitte_e', 'nishi_umeda_08'], ['nishi_umeda_05', 'nishi_umeda_02'], ['nishi_umeda_05', 'nishi_umeda_03'], ['nishi_umeda_06', 'nishi_umeda_05'], ['nishi_umeda_07', 'nishi_umeda_06'], ['nishi_umeda_08', 'nishi_umeda_07'], ['nishi_umeda_08', 'nishi_umeda_09'], ['nishi_umeda_09', 'j_nishi_x'], ['nishi_umeda_11', 'j_kitte_e'], ['nishi_umeda_11', 'nishi_umeda_10'], ['nishi_umeda_12', 'nishi_umeda_14'], ['nishi_umeda_13', 'nishi_umeda_11'], ['nishi_umeda_13', 'nishi_umeda_15'], ['nishi_umeda_14', 'nishi_umeda_13'], ['nishi_umeda_16', 'nishi_umeda_14'], ['nishi_umeda_16', 'nishi_umeda_17'], ['nishi_umeda_03', 'nishi_umeda_p2_garden'], ['nishi_umeda_p2_garden', 'nishi_umeda_04'], ['garden', 'nishi_umeda_p2_garden'], ['nishi_umeda_01', 'nishi_umeda_p2_ritz'], ['ritz', 'nishi_umeda_p2_ritz'], ['nishi_umeda_02', 'nishi_a1_j'], ['nishi_a1_j', 'nishi_umeda_01'], ['nishi_umeda_p2_ritz', 'nishi_y'], ['nishi_y', 'nishi_y_k1'], ['nishi_y_k1', 'exit_6_1'], ['nishi_y', 'exit_6_2']] },
  { floor: 'B1', zone: 'nishi_umeda', pts: [[699.7, 1321.2], [695.1, 1321.7], [694.7, 1328.8], [689.2, 1329.8], [688.7, 1333.4], [693.4, 1332.5], [697.2, 1325.9], [699.4, 1332.1], [703.0, 1331.8], [703.3, 1334.6], [706.7, 1330.5], [716.9, 1324.6], [713.3, 1315.9], [721.7, 1310.3], [715.8, 1296.5], [718.5, 1294.9], [719.7, 1267.5], [717.2, 1261.9], [714.3, 1281.1], [710.3, 1281.6], [705.1, 1298.3], [705.1, 1308.5], [666.7, 1219.5], [657.0, 1221.8]] },
  { floor: 'B1', zone: 'nishi_umeda', pts: [[555.4, 1268.1], [560.9, 1274.6], [566.6, 1270.1], [551.8, 1259.5], [546.4, 1264.6], [556.0, 1267.2]] },
  { floor: 'B1', zone: 'nishi_umeda', pts: [[335.3, 1344.4], [335.7, 1352.5], [343.7, 1353.4], [343.4, 1344.9]] },
  { floor: 'B1', zone: 'nishi_umeda', pts: [[554.3, 1247.3], [547.1, 1247.0], [547.1, 1249.7], [551.4, 1254.1], [554.5, 1254.1]] },
  { floor: 'B1', zone: '_neutral', pts: [[1076.7, 1219.7], [1092.6, 1259.1], [1090.6, 1259.3], [1091.6, 1262.1], [1093.7, 1261.7], [1116.0, 1316.9], [1119.9, 1316.8], [1110.4, 1318.5], [1123.1, 1325.7], [1126.4, 1334.7], [1133.9, 1337.2], [1081.0, 1206.2], [1062.7, 1126.1], [1088.8, 1116.9], [1090.9, 1123.4], [1100.1, 1119.5], [1098.3, 1112.9], [1107.1, 1110.0], [1108.0, 1112.5], [1112.1, 1111.3], [1111.3, 1108.9], [1115.6, 1107.4], [1114.6, 1103.3], [1109.5, 1104.7], [1108.8, 1103.0], [1089.8, 1110.0], [1084.2, 1085.6], [1079.6, 1086.0], [1085.5, 1108.2], [1060.5, 1116.5], [1059.1, 1110.5], [1050.4, 1112.8], [1051.9, 1119.4], [1034.1, 1125.3], [1038.3, 1136.1], [1042.6, 1134.9], [1045.7, 1135.9], [1044.5, 1132.6], [1054.1, 1129.2], [1056.3, 1140.2], [1079.1, 1208.1], [1070.9, 1217.6], [1068.6, 1217.6], [1079.8, 1250.4], [1085.6, 1254.4], [1074.1, 1220.6]], covers: [['j_f40', 'j_mido_s'], ['j_mido_s', 'sonechika']] },
  ...
  { floor: 'B2', zone: 'nishi_umeda', pts: [[710.1, 1183.2], [724.8, 1179.3], [724.9, 1178.0], [720.8, 1177.9], [720.8, 1179.3], [711.9, 1176.7], [731.6, 1171.5], [729.3, 1162.8], [704.2, 1169.5], [703.6, 1179.0], [709.9, 1177.3]], covers: [['hilton_b2', 'herbis_b2']] },
  { floor: 'B2', zone: 'nishi_umeda', pts: [[710.2, 1187.6], [710.4, 1193.5], [715.8, 1192.1], [715.8, 1190.6], [725.5, 1189.5], [725.4, 1188.0], [716.5, 1187.8], [715.6, 1182.7], [715.8, 1187.8]] },
];
...
// 点ごとの深さ(m)。現地記録の階段・坂の途中の点(tools/data/survey/*.json)。無い点は施設×階の深さ(zoneDepth)。
// 分岐点(J)は zone を持たないので、つながる通路の zone から推定する
const NODE_DEPTH = {
...
const hdist = (a, b) => Math.hypot(a.x - b.x, a.z - b.z); // 水平距離(誇張した高さを距離に乗せない)
...
function updateZoneLabels() {
  for (const { id, lab } of zoneLabelObjs) {
    ...
    lab.visible = !!target;
    if (target) lab.position.y = yOfZone(id, target) + 14;
  }
}
updateZoneLabels();
...
for (const fp of FLOOR_POLYS) {
  ...
  mesh.position.y = yOfZone(fp.zone, fp.floor) - 1.5; // ゾーン間の重なりは生成時に除去済み。高さは施設×階の深さ(既定は層の高さ)
...
for (const v of VERTICALS) {
  ...
  const yB1 = Math.max(nodeById[v.a].y, nodeById[v.b].y);
  const yB2 = Math.min(nodeById[v.a].y, nodeById[v.b].y);
...
for (const pl of PLAZAS) {
  ...
  mesh.position.set(x, yOfZone(pl.zone, pl.floor) - 0.08, z);
  floorGroups[pl.floor].add(mesh);
}

// スポット（クリック対象）・ジャンクション・店舗
const nodeMeshes = [];
window.__dbg.nodeMeshes = nodeMeshes; window.__dbg.floorGroups = floorGroups; window.__dbg.enterDetail = enterDetail; window.__dbg.exitDetail = exitDetail; // 開発用: タップ判定の再現・詳細地図の確認に使う
const labelDivs = {};
...
for (const md of MERGED_DOTS) {
  ...
  mesh.position.set(x, yOfZone(md.zone, md.floor) + 1.7, z);
...
const detailLinkMat = new THREE.MeshStandardMaterial({ color: 0xffd76a, emissive: 0x7a5410, roughness: 0.4, side: THREE.DoubleSide }); // 他施設への接続の矢印
for (const [zone, M] of Object.entries(DETAIL_MAPS)) {
  ...
  // 他の施設への接続(階段・通路の出口): 金色の矢印と「○○へ」のラベル(detail_maps.js の links、ガイド座標)。
  ...
  for (const L of M.links || []) {
    ...
    geo.rotateX(-Math.PI / 2); // shape +y → world -z(北)。dir(guide: +y=南=world +z)へ回す
    const mesh = new THREE.Mesh(geo, detailLinkMat);
...
if (IS_SURVEY) {
  ...
  for (const lm of LANDMARKS) {
    ...
    if (lm.frame) {
      ...
      parent = floorGroups[lm.floor]; y = yOfZone(lm.zone, lm.floor) + 1.6; top = GROUND_Y;
...
for (const [ia, ib] of EDGES) {
  const w = hdist(nodeById[ia], nodeById[ib]) * UNIT_M; // 水平距離(高低差の誇張を距離に乗せない)
  adj[ia].push({ to: ib, w });
  adj[ib].push({ to: ia, w });
}
// ホール内部の通路格子（描画されないが歩ける）
for (const [ia, ib] of HALL_EDGES) {
  const w = hdist(nodeById[ia], nodeById[ib]) * UNIT_M;
  adj[ia].push({ to: ib, w });
  adj[ib].push({ to: ia, w });
}
...
function showRoute(startId, goalId) {
  ...
  for (let i = 0; i < path.length; i++) {
    ...
    if (i > 0) {
      ...
      if (prevN.floor !== n.floor) {
        ...
        if (vp) {
          pts.push(new THREE.Vector3(vp.x, prevN.y + 7, vp.z));
          pts.push(new THREE.Vector3(vp.x, n.y + 7, vp.z));
        }
      }
    }
  ...
  for (let i = 1; i < path.length; i++) {
    ...
    if (prevN.floor === n.floor) {
      ...
      const dm = hdist(prevN, n) * UNIT_M; // 水平距離(同一層内の階段・坂の高さは乗せない)
      leg += dm; walked += dm;
    ...
    if (prevN.floor !== n.floor) {
      ...
      const dirWord = n.y < prevN.y ? '下りる' : '上がる';
...
if (new URLSearchParams(location.search).get('survey') === '1') {
  ...
  survey = initSurvey({
    ...
    yOf: yOfZone, // 施設×階の床の高さ(高低差モデル)。マーカーを床に合わせる
