...
const NODES = [
  ...
  P('dojima_flat', '堂島ふらっと (近鉄堂島ビルB1F)', 'B1', 668, 1518, 'dojima_flat'), // ドーチカ C83 の階段を上がった所(現地 2026-09-03)
  // ドーチカ⇔堂島ふらっとの階段(現地の2点タップ 2026-09-03、tools/data/survey/2026-09-03_dotica_dojima_flat.json)。
  // 23段(≈3.9m上り)。ドーチカから見て左が階段、右(北側)に上下2本のエスカレーター。同じ「中枢層」扱いなので昇降設備にはせず、通路の折れ点として持つ
  J('dotica_df_s0', 692.9, 1525.8), // 階段の下(ドーチカ側)
  J('dotica_df_s1', 681.1, 1519.9), // 階段の上(堂島ふらっと側)
  P('kanden_b1',   '関電不動産西梅田ビル B1F',    'S1', 688, 1380, 'kanden'), // 館内階段でB2Fと(現地 2026-09-03)
  P('kanden_b2',   '関電不動産西梅田ビル B2F',    'B1', 688, 1380, 'kanden'), // ドーチカ C61 の通路を入った所(北東角)
  ...
  J('dotica_c83',    702.7, 1527),   // ドーチカ通路の C83(堂島ふらっとへの階段)の分岐点
  // ドーチカ⇔アバンザの接続(現地の2点タップ 2026-09-03、tools/data/survey/2026-09-03_dotica_avanza.json)。1段15cm
  //   中: 通路の東側(707.7,1507)から7段上がる→10m平坦→17段上がる→アバンザ(745.6,1501.3)。合計3.6m上り
  //   南: (713.1,1540.9)から東へ10段(1.5m)上がる→館内の南側通路へ
  J('dotica_avz_c', 703.9, 1507),    // 中: ドーチカ通路上の分岐点(dotica_avz_n→dotica_c83 の線上)
  J('avz_c_s',      707.7, 1507),    // 中: 7段の階段の下(通路の東縁)
  J('avz_c_mid',    728, 1503.9),    // 中: 平坦部(7段の上〜17段の下)
  J('j_avz_c',      745.6, 1501.3),  // 中: 17段の階段の上(アバンザ館内)
  J('avz_s0',       713.1, 1540.9),  // 南: 10段の階段の下(ドーチカ側)
  J('avz_s1',       718.8, 1538.9),  // 南: 10段の階段の上
  J('j_avz_s',      748, 1532),      // 南: アバンザ館内の南側通路の入口
...
const EDGES = [
  ...
  ['dotica_02', 'kanden_b2', 5, 'dotica'],      // C61: ドーチカ北側の西へ出る通路→関電不動産西梅田ビル B2F(北東角)
  ...
  ['dotica_avz_n', 'dotica_avz_c', 12, 'dotica'], // osm 1010195561
  ['dotica_avz_c', 'dotica_c83', 12, 'dotica'], // osm 1010195561
  ['dotica_c83', 'dotica_01', 12, 'dotica'], // osm 1010195561
...
const VERTICALS = [
  ...
  { type: 'stairs', a: 'kanden_b1',   b: 'kanden_b2',   mx: 688, my: 1385, name: '関電不動産西梅田ビル館内階段' }, // B2案内板の「現在地」横の階段(東側)
...
const FLOOR_POLYS = [
  ...
  { floor: 'B1', zone: 'avanza', pts: [[744.5, 1530.9], [823.2, 1542.6], [831.8, 1485.4], [833.9, 1483.1], [832.2, 1482.8], [833.0, 1477.7], [828.0, 1482.2], [754.3, 1471.1], [756.0, 1474.2], [750.8, 1468.7], [741.2, 1532.8]], covers: [['j_avz_s', 'j_avz'], ['j_avz_c', 'j_avz'], ['j_avz_n', 'j_avz']] },
  ...
  { floor: 'B1', zone: 'dotica', pts: [[679.0, 1522.6], [696.4, 1530.9], [695.1, 1534.5], [695.5, 1545.2], [696.7, 1545.3], [710.2, 1637.3], [722.0, 1635.6], [709.6, 1544.8], [751.0, 1533.9], [750.6, 1531.9], [744.6, 1531.0], [741.2, 1532.9], [741.3, 1531.0], [712.8, 1538.4], [709.1, 1537.9], [709.8, 1509.5], [744.9, 1503.9], [745.6, 1498.8], [710.1, 1504.1], [711.3, 1485.5], [748.1, 1485.5], [749.2, 1478.5], [711.7, 1478.5], [719.0, 1339.0], [717.3, 1338.9], [717.5, 1334.7], [708.5, 1334.3], [708.3, 1338.4], [707.0, 1338.3], [705.3, 1369.5], [698.5, 1372.6], [698.1, 1378.2], [705.0, 1375.1], [696.8, 1523.3], [684.5, 1517.4], [678.8, 1516.5]], covers: [['j_sone_w', 'dojima'], ['dotica_01', 'avz_s0'], ['avz_s0', 'avz_s1'], ['avz_s1', 'j_avz_s'], ['dotica_avz_n', 'j_avz_n'], ['dotica_avz_c', 'avz_c_s'], ['avz_c_s', 'avz_c_mid'], ['avz_c_mid', 'j_avz_c'], ['dotica_c83', 'dotica_df_s0'], ['dotica_df_s0', 'dotica_df_s1'], ['dotica_df_s1', 'dojima_flat'], ['dotica_02', 'kanden_b2'], ['dotica_02', 'dojima'], ['dojima', 'dotica_avz_n'], ['dotica_avz_n', 'dotica_avz_c'], ['dotica_avz_c', 'dotica_c83'], ['dotica_c83', 'dotica_01'], ['dotica_01', 'avanza'], ['avanza', 'dotica_03']] },
...
const NODE_DEPTH = {
  ...
  dotica_avz_n: 6, j_avz_n: 2.4, // 北の接続は未記録(仮)
};
{
  const zoneGuess = {};
