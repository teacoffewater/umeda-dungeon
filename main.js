...
const NODES = [
  ...
  P('dojima',      'ドージマ地下センター',       'B1',  703.5, 1446, 'dotica'),
  P('avanza',      '堂島アバンザ前',             'B1',  702.5, 1543.3, 'dotica'),
  P('dojima_flat', '堂島ふらっと (近鉄堂島ビルB1F)', 'B1', 655.7, 1520.9, 'dojima_flat'), // ドーチカ C83 の通路の西端(板)。階段を上がった所(現地 2026-09-03)
  ...
  J('dotica_df_s0', 666.4, 1522.5), // 階段の下(ドーチカ側)
  J('dotica_df_s1', 661, 1521.7),   // 階段の上(堂島ふらっと側)
  P('kanden_b1',   '関電不動産西梅田ビル B1F',    'S1', 683.3, 1381.1, 'kanden'), // 館内階段でB2Fと(現地 2026-09-03)
  P('kanden_b2',   '関電不動産西梅田ビル B2F',    'B1', 683.3, 1381.1, 'kanden'), // ドーチカ C61 の通路(板)を西へ入った所(北東角)
  ...
  J('dotica_avz_n', 702.9, 1460.5),  // 北: ドーチカ本線の C72 の通路の分岐点
  J('avz_n_s',      741.7, 1466.4),  // 北: 階段の下(C72 の通路の東端手前)
  J('j_avz_n',      753.4, 1468.7),  // 北: 階段の上(アバンザ北サンクンガーデン通路)
  J('dotica_c83',   697.8, 1527.2),  // ドーチカ本線の C83(堂島ふらっとへの通路)の分岐点
  // 中: 通路の東側から7段上がる→10m平坦(広場)→17段(曲線階段)上がる→アバンザ。合計3.6m上り(1段15cm)
  // 南: 東へ10段(1.5m)上がる→館内の南側通路へ
  J('dotica_avz_c', 700.8, 1510.6),  // 中: ドーチカ本線の C80 の通路の分岐点
  J('avz_c_s',      714.8, 1512.7),  // 中: 7段の階段の下(区画帯の東縁)
  J('avz_c_mid',    720.1, 1515.5),  // 中: 広場(7段の上〜17段の下)
  J('j_avz_c',      723, 1506.6),    // 中: 曲線階段(17段)の上(アバンザ B1F の面)
  J('avz_s0',       715.6, 1538.1),  // 南: 10段の階段の下(ドーチカ側)
  J('avz_s1',       721, 1536.8),    // 南: 10段の階段の上
  J('j_avz_s',      748.7, 1531.4),  // 南: アバンザ館内の南側通路の入口(ビルの南西角)
  ...
  J('dotica_02',  711.1, 1368.2),  // 北端(曽根崎地下歩道 j_sone_w との境)
  J('dotica_c61', 708.6, 1384.8),  // C61 の通路(西→関電不動産西梅田ビル B2F)の分岐点
  J('dotica_01',  702.4, 1536.1),  // C84 の通路(東→アバンザ南)の分岐点
  J('dotica_c92', 710.8, 1595.9),  // C92 の所の折れ(本線が東へ寄る)
  J('dotica_03',  701.5, 1610.6),  // 南端(C93 の手前。板の通路はここまで)
...
const VERTICALS = [
  ...
  { type: 'stairs', a: 'kanden_b1',   b: 'kanden_b2',   mx: 681, my: 1385, name: '関電不動産西梅田ビル館内階段' }, // B2案内板の「現在地」横の階段(東側)
...
const FLOOR_POLYS = [
  ...
  { floor: 'B1', zone: 'avanza', pts: [[749.9, 1540.3], [752.5, 1531.6], [823.2, 1542.6], [831.8, 1485.4], [833.9, 1483.1], [832.2, 1482.8], [833.0, 1477.7], [828.0, 1482.2], [763.5, 1472.4], [763.6, 1470.5], [756.5, 1468.9], [714.6, 1463.1], [713.4, 1515.3], [714.9, 1515.6], [716.5, 1519.6], [717.5, 1525.9], [716.2, 1535.4]], covers: [['j_avz_s', 'j_avz'], ['j_avz_c', 'j_avz'], ['j_avz_n', 'j_avz']] },
  ...
  { floor: 'B1', zone: 'dotica', pts: [[679.2, 1528.6], [687.8, 1528.7], [695.3, 1594.7], [696.2, 1600.9], [697.8, 1601.4], [697.8, 1605.2], [693.2, 1612.5], [703.4, 1618.9], [707.2, 1613.5], [720.3, 1613.5], [719.1, 1594.0], [715.1, 1570.2], [712.8, 1569.8], [708.4, 1539.5], [717.7, 1540.9], [730.2, 1537.6], [711.2, 1534.9], [710.3, 1514.6], [713.3, 1515.0], [714.4, 1463.1], [760.3, 1469.9], [756.5, 1468.3], [756.9, 1466.3], [746.4, 1464.3], [741.5, 1462.3], [740.7, 1459.8], [730.6, 1456.8], [729.0, 1456.6], [728.4, 1461.3], [713.6, 1458.6], [720.1, 1385.7], [717.2, 1384.7], [717.3, 1379.4], [715.7, 1378.0], [718.3, 1368.5], [720.5, 1368.8], [721.1, 1367.4], [720.8, 1354.7], [716.4, 1354.1], [717.6, 1334.8], [708.6, 1334.2], [707.5, 1352.7], [691.5, 1350.4], [691.5, 1352.1], [698.5, 1353.7], [698.0, 1361.6], [699.6, 1363.9], [699.5, 1369.1], [693.5, 1420.6], [695.8, 1436.4], [692.4, 1436.2], [689.0, 1474.5], [696.7, 1474.5], [688.6, 1480.4], [687.0, 1519.8], [678.9, 1518.9]], covers: [['j_sone_w', 'dotica_02'], ['dotica_01', 'avz_s0'], ['avz_s0', 'avz_s1'], ['avz_s1', 'j_avz_s'], ['dotica_avz_n', 'avz_n_s'], ['avz_n_s', 'j_avz_n'], ['dotica_avz_c', 'avz_c_s'], ['avz_c_s', 'avz_c_mid'], ['avz_c_mid', 'j_avz_c'], ['dotica_c83', 'dotica_df_s0'], ['dotica_df_s0', 'dotica_df_s1'], ['dotica_df_s1', 'dojima_flat'], ['dotica_c61', 'kanden_b2'], ['dotica_02', 'dotica_c61'], ['dotica_c61', 'dojima'], ['dojima', 'dotica_avz_n'], ['dotica_avz_n', 'dotica_avz_c'], ['dotica_avz_c', 'dotica_c83'], ['dotica_c83', 'dotica_01'], ['dotica_01', 'avanza'], ['avanza', 'dotica_c92'], ['dotica_c92', 'dotica_03']] },
  ...
  { floor: 'B1', zone: 'ekimae', pts: [[853.2, 1251.1], [740.5, 1252.0], [727.5, 1264.5], [725.4, 1310.6], [735.7, 1321.3], [712.9, 1334.3], [717.7, 1334.7], [717.3, 1341.0], [741.4, 1327.2], [840.0, 1334.7], [857.0, 1349.1], [857.5, 1344.4], [860.0, 1344.5], [860.0, 1341.2], [852.2, 1334.5], [859.8, 1327.5], [861.2, 1260.3]] },
  ...
  { floor: 'B1', zone: 'sonechika', pts: [[717.1, 1345.6], [852.2, 1357.6], [851.9, 1362.9], [829.8, 1361.2], [828.8, 1374.1], [893.3, 1379.2], [894.4, 1366.3], [864.8, 1363.9], [865.1, 1359.5], [856.1, 1358.5], [857.0, 1349.3], [851.2, 1344.4], [730.4, 1333.7], [717.3, 1341.2]] },
  ...
  { floor: 'B1', zone: 'nishi_umeda', pts: [[657.0, 1221.8], [708.0, 1340.6], [707.2, 1337.5], [708.5, 1334.1], [712.7, 1334.3], [713.5, 1332.1], [719.9, 1330.2], [722.8, 1257.7], [709.8, 1257.2], [707.5, 1314.1], [666.7, 1219.5]] },
